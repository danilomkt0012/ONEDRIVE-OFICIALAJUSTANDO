...
- **Dispatch Modes (Seguro/Equilibrado/Turbo)**: User-selectable dispatch profile per campaign (`campaigns.dispatch_mode`). Each mode tunes refill rate, concurrency, human delays and pause windows. ETA preview shown in wizard via `GET /api/dispatch/eta`. NEVER auto-pauses by quality drop — degraded numbers only get reduced rotation weight (min 5%). Defined in `server/services/engine/DispatchProfile.ts`.
- **Adaptive Sender Scoring (0–100)**: Every number has a confidence score recomputed every 50 sends from delivery rate, errors, replies, and quality changes. Score drives weighted round-robin distribution with 5% minimum share so new numbers always get traffic. Persisted in `waba_numbers.confidence_score` + history table `sender_score_history`. Health summary at `GET /api/dispatch/sender-health`. Defined in `server/services/engine/AdaptiveScoring.ts`.
- **Per-User Frequency Cap Blacklist**: When Meta returns error 131049/131056 for a recipient, that phone is added to `frequency_blacklist` for 7 days and skipped on subsequent campaigns automatically. Implemented in `OptOutService.addToFrequencyBlacklist`.
